...
from spiders.ucc_spider import UccSpider
from mock import MagicMock


class TestUccSpider(unittest.TestCase):

    #def setUp(self):

    def test_load_vendor_info_to_db_success(self):
        curmock = MagicMock()
        curmock.execute = MagicMock()
        curmock.fetchone = MagicMock(side_effect=[[0], [0]])

        vendorsFile = "../sp_vendors.json"

        UccSpider.loadVendorInfoFromFileToDB(vendorsFile)
